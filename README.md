...
- **Environment-scoped test data**: `utils/testData.ts` loads `src/test-data/<ENV>/<filename>.json`, so test data can differ per environment. Add JSON files there as needed. For this practice suite, `prod` is not added to `.gitignore` — its only data (`card.json`) is a well-known public Stripe test card, not a real credential, so it's safe to commit. On a project with real prod secrets, `prod` should be the one git-ignored instead; `src/test-data/stage/` is git-ignored here (aside from `.gitkeep`) to demonstrate that pattern.
